...
/*
 * Points to current test-case access "kernels". This should usually just be
 * current test-case itself.
 */
...
static void probe_console(void *ignore, const char *buf, size_t len)
{
	...
	if (strnstr(buf, "BUG: KCSAN: ", len) && strnstr(buf, "test_", len)) {
		...
		strscpy(observed.lines[0], buf, sizeof(observed.lines[0]));
		observed.nlines = 1;
	} else if (observed.nlines <= 2 && strnstr(buf, "bytes by", len)) {
		strscpy(observed.lines[observed.nlines++], buf, sizeof(observed.lines[0]));

		if (strnstr(buf, "race at unknown origin", len)) {
			if (WARN_ON(observed.nlines != 2))
				goto out;

			/* No second line of interest. */
			strcpy(observed.lines[observed.nlines++], "<none>");
		}
	}
...
/* Information we expect to see in a report. */
struct expect_report {
	struct {
		void *fn;    /* Function we expect to see in report. */
...
{
	const bool is_assert = (r->access[0].type & KCSAN_ACCESS_ASSERT) ||
			       (r->access[1].type & KCSAN_ACCESS_ASSERT);
	bool ret = false;
	unsigned long flags;
	...
	/* Doubled-checked locking; data race ok, rechecked below. */
	if (READ_ONCE(observed.nlines) != ARRAY_SIZE(observed.lines))
		return false;
	...
	spin_lock_irqsave(&observed.lock, flags);
	if (observed.nlines != ARRAY_SIZE(observed.lines))
		goto out;
...
static void test_basic(struct kunit *test)
{
	...
	} while (!end_test_checks(match_expect || match_never));
...
static void test_concurrent_races(struct kunit *test)
{
	...
	static const struct expect_report never = {
		.access = {
			{ test_kernel_rmw_array, &test_array, sizeof(long), 0 },
			{ test_kernel_rmw_array, &test_array, sizeof(long), 0 },
		},
	};
	bool match_expect = false;
	...
	do {
		...
		/*
		 * Stress KCSAN with lots of concurrent races on different
		 * addresses until timeout.
		 */
	} while (!end_test_checks(false));
	/* Sanity check we had matches. */
	KUNIT_EXPECT_TRUE(test, match_expect);
	KUNIT_EXPECT_FALSE(test, match_never);
}
...
static void test_read_atomic_write_atomic(struct kunit *test)
{
	const struct expect_report never = {
		.access = {
			{ test_kernel_read_atomic, &test_var, sizeof(test_var), KCSAN_ACCESS_ATOMIC },
...
static void test_data_race(struct kunit *test)
{
	const struct expect_report never = {
		.access = {
			{ test_kernel_data_race, &test_var, sizeof(test_var), KCSAN_ACCESS_WRITE },
			{ test_kernel_data_race, &test_var, sizeof(test_var), 0 },
		},
	};
	bool match_never = false;
...
static void test_assert_exclusive_bits_change(struct kunit *test)
{
	const struct expect_report expect = {
		.access = {
			...
			{ test_kernel_change_bits, &test_var, sizeof(test_var), KCSAN_ACCESS_WRITE | KCSAN_ACCESS_ATOMIC },
		},
	};
	bool match_expect = false;
...
static void test_assert_exclusive_bits_nochange(struct kunit *test)
{
	const struct expect_report never = {
		.access = {
			{ test_kernel_assert_bits_nochange, &test_var, sizeof(test_var), KCSAN_ACCESS_ASSERT },
			{ test_kernel_change_bits, &test_var, sizeof(test_var), KCSAN_ACCESS_WRITE | KCSAN_ACCESS_ATOMIC },
		},
	};
	bool match_never = false;
...
static void test_seqlock_noreport(struct kunit *test)
{
	const struct expect_report expect = {
		.access = {
			{ test_kernel_seqlock_reader, &test_var, sizeof(test_var), 0 },
			{ test_kernel_seqlock_writer, &test_var, sizeof(test_var), KCSAN_ACCESS_WRITE },
		},
	};
	bool match_expect = false;
...
static int access_thread(void *arg)
{
	...
	do {
		if (!timer_pending(&timer))
			mod_timer(&timer, jiffies + usecs_to_jiffies(10));
		else {
...
static struct kunit_suite kcsan_test_module = {
	.name = "kcsan-test",
...
static int __init kcsan_test_init(void)
{
	...
	kunit_run_tests(&kcsan_test_module);
	return 0;
}

static void kcsan_test_cleanup(void)
{
...
module_exit(kcsan_test_cleanup);
