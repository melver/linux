 ...
 * architectures implement their atomics using the compilers' builtins.
 *
 * If an architecture decides to implement atomics using the builtins, it should
 * be noted that, because they are implicitly instrumented by KCSAN (and KASAN,
 ...
 *	    }
 *
 * The only downside is that, if we there are 3 threads, with one CAS that
 * succeds, another CAS that fails, and an unmarked racing operation, we may
