...
/*
 * Wrapper to type-check requirements: __reorder_access_cmp contains a non-NULL
 * value only iff it was matched via get_reorder_access_cmp().
 * Safely avoids repeated calls to get_reorder_access_cmp().
 */
struct __reorder_access_cmp { struct kcsan_scoped_access *v; };
static __always_inline struct __reorder_access_cmp
get_reorder_access_cmp(struct kcsan_ctx *ctx, const volatile void *ptr,
		       size_t size, int type, unsigned long ip)
{
	struct kcsan_scoped_access *reorder_access = get_reorder_access(ctx);

	if (!reorder_access)
		return (struct __reorder_access_cmp){ .v = NULL };
	...
	return (reorder_access->ptr == ptr && reorder_access->size == size &&
		reorder_access->type == type && reorder_access->ip == ip)
			? (struct __reorder_access_cmp){ .v = reorder_access }
			: (struct __reorder_access_cmp){ .v = NULL };
}

static inline void invalidate_reorder_access(struct kcsan_ctx *ctx)
{
	struct kcsan_scoped_access *reorder_access = get_reorder_access(ctx);
...
set_reorder_access(struct kcsan_ctx *ctx, const volatile void *ptr, size_t size,
		   int type, unsigned long ip, unsigned long access_mask)
{
	struct kcsan_scoped_access *reorder_access = get_reorder_access(ctx);
	...
	reorder_access->access_mask	= access_mask;
	reorder_access->pin_count	= 0;
}

static __always_inline unsigned long
get_access_mask(struct kcsan_ctx *ctx, struct __reorder_access_cmp reorder_access)
{
	return reorder_access.v ? reorder_access.v->access_mask : ctx->access_mask;
}
					    ...
					    unsigned long ip)
{
	...
	struct __reorder_access_cmp reorder_access_cmp;
	struct kcsan_ctx *ctx = get_ctx();
	...
	reorder_access_cmp = get_reorder_access_cmp(ctx, ptr, size, type, ip);
	access_mask = get_access_mask(ctx, reorder_access_cmp);
...
kcsan_setup_watchpoint(const volatile void *ptr, size_t size, int type, unsigned long ip)
{
	 ...
	 *
	 *	2. Races of the reordered access from nested interrupts. Assume
	 *	   setting up a watchpoint for a non-reordered (normal) access
	 *	   that also conflicts with the current ctx->reorder_access.
	 *	   Upon a nested interrupt, which shares the context, it would
	 *	   check a conflicting ctx->reorder_access (which originated
	 *	   from the parent's execution). To avoid, ctx->reorder_access
	 *	   is invalidated (reset later, and unused until then).
	 */
	if (reorder_access_cmp.v)
		interrupt_watcher = false;
	invalidate_reorder_access(ctx);
	...
	set_reorder_access(ctx, ptr, size, type, ip, access_mask);
out_unlock:
	if (!interrupt_watcher)
...
#define DEFINE_MEMORY_BARRIER(name, order_before_cond)                         \
	noinstr void __kcsan_##name(void)                                      \
	{                                                                      \
		struct kcsan_scoped_access *sa;                                \
 ...
 */
noinstr void __tsan_func_entry(void *call_pc)
{
	if (within_noinstr(_RET_IP_))
		return;

	instrumentation_begin();
	get_reorder_access(get_ctx())->pin_count++;
	instrumentation_end();
}

noinstr void __tsan_func_exit(void)
{
	...
	if (--reorder_access->pin_count < 0) {
		/*
		 * Last access check to catch cases where supposed release was
		 * last access in function. Because instrumentation happens
		 * before the actual access, a conflicting access due to the
		 * release giving up a c-s would only be caught if we do the
		 * conflicting access after the supposed release.
		 */
