...
static int bpf_user_writable_entry_cmp(const void *_key, const void *_ent)
{
...
static const struct bpf_user_writable_entry *bpf_user_writable_find(void __user *ptr, size_t size)
{
	...
	return bsearch(&key, uw->entries, uw->size, sizeof(key), bpf_user_writable_entry_cmp);
}
...
int bpf_user_writable_register(void __user *start, size_t size, u32 tag)
{
	 ...
	 */
	if (bpf_user_writable_find(start, 1))
		return -EEXIST;

	if (current->bpf_user_writable) {
		uw = current->bpf_user_writable;
	} else { /* initial alloc */
		uw = kmalloc(sizeof(*uw), GFP_KERNEL);
		if (!uw)
			return -ENOMEM;
		uw->capacity = 8; /* small initial capacity */
		uw->size = 0;
		uw->entries = kmalloc_array(uw->capacity, sizeof(*uw->entries), GFP_KERNEL);
		if (!uw->entries) {
			kfree(uw);
			return -ENOMEM;
		}
	...
	if (uw->capacity == uw->size) { /* grow exponentially */
		const size_t ncap = uw->capacity * 2;
		struct bpf_user_writable_entry *narr =
			krealloc_array(uw->entries, ncap, sizeof(*narr), GFP_KERNEL);
...
void bpf_user_writable_free(struct task_struct *t)
{
	...
	WARN_ON(!t->bpf_user_writable->entries);

	kfree(t->bpf_user_writable->entries);
