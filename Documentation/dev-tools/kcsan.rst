...
barriers. Specifically, with ``CONFIG_KCSAN_STRICT=y``, KCSAN models load or
store buffering, and can detect missing ``smp_mb()``, ``smp_wmb()``,
...
KCSAN's approach to detecting missing memory barriers is based on modeling
access reordering (only enabled if ``CONFIG_KCSAN_STRICT=y``). Each memory
access for which a watchpoint is set up, is also selected for simulated
reordering within the scope of its function (at most 1 in-flight access).

We are limited to modeling the effects of "buffering" (delaying the access),
since the runtime cannot "prefetch" accesses (the reason why acquires remain
entirely uninstrumented). Once an access has been selected for reordering, it
is checked along every other access until the end of the function scope. If an
...
5. **Memory Ordering:** KCSAN is aware only of a subset of LKMM ordering rules;
