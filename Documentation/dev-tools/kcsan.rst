...
* ``kcsan.weak_memory`` (default on, depends on ``CONFIG_KCSAN_STRICT=y``):
  Enables `modeling weak memory`_, allowing to detect data races due to missing
  memory barriers.
...
based on modeling access reordering (enabled if ``CONFIG_KCSAN_STRICT=y``).
...
Furthermore, with the currently available compiler instrumentation, the
